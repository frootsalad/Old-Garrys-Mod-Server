...
function Prometheus.DB.FetchPlayerBought(Callback, Ply)
	...
	Prometheus.DB.Query("SELECT a.active, a.expiretime, p.title FROM actions a INNER JOIN packages p ON a.package = p.id WHERE a.uid = '" .. Ply:SteamID64() .. "' AND (a.server LIKE '%\"" .. Prometheus.ServerID .. "\"%' OR a.server LIKE '%\"0\"%') GROUP BY a.active, a.expiretime, p.title, a.timestamp DESC", LCallback)
end
