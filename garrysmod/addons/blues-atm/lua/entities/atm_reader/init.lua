	...
	Accounts.GetCachedPersonalAccount(ply:SteamID64(), function(account, didExist)
		if didExist then
			...
			if account.balance - ent:GetItemPrice() >= 0 then
				--Okay, they have enough so take the money
				...
				Accounts.GetCachedPersonalAccount(ent:GetReaderOwner():SteamID64(), function(account2, didExist)
					if didExist then
						...
						--Check balance again, then take the money
						if account.balance - ent:GetItemPrice() < 0 then
						...
						--The other account failed, so instead we refund there balance
						account:AddBalance(ent:GetItemPrice() * 1, BATM.Lang["Refund from a corrupt transaction."])							
